...
export class AtlasManager {
    ...
    async updateAtlas() {
        ...
        if (!bookName || !this.mapDataManager.isInitialized()) {
            ...
            // If the book exists but data isn't initialized, we should still clear the Atlas.
            if (bookName) {
                const worldbook = await this.lorebookManager.TavernHelper.getWorldbook(bookName);
                 if (worldbook.some(e => e.name === this.ATLAS_ENTRY_NAME)) {
                    await this.lorebookManager.updateEntryContent(bookName, this.ATLAS_ENTRY_NAME, '[System Note: No map data loaded.]', false);
                 }
            }
            return;
        }

        // 1. Build the tree structure and content string
        const content = this._buildAtlasContent();
        
        // 2. Update the lorebook entry
        const worldbook = await this.lorebookManager.TavernHelper.getWorldbook(bookName);
        const atlasEntry = worldbook.find(e => e.name === this.ATLAS_ENTRY_NAME);

        if (!atlasEntry) {
            this.logger.log(`[AtlasManager] Creating new atlas entry...`);
            await this.lorebookManager.createEntry(bookName, {
                name: this.ATLAS_ENTRY_NAME,
                content: content,
                comment: 'A high-level overview of the entire known world. Auto-generated by The World plugin. Do not edit manually.',
                strategy: { type: 'constant' }, // Always active
                position: { type: 'before_character_definition', order: -2, role: 'system' } // High priority
            });
        } else {
            // Only update if content has actually changed
            if (atlasEntry.content !== content) {
                await this.lorebookManager.updateEntryContent(bookName, this.ATLAS_ENTRY_NAME, content, false);
            } else {
                this.logger.log('[AtlasManager] Atlas content is already up-to-date.');
            }
        }
