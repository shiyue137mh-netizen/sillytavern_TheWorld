...
// GlobalThemeManager is removed as its logic is now integrated here.
...
export class TheWorldApp {
    ...
    async initialize() {
        ...
        // Directly instantiate TimeGradient
        this.timeGradient = new TimeGradient(dependencies);
        ...
        this.uiController = new UIController({ 
            ...
            // Pass the timeGradient instance directly for toggle switch logic
        ...
        if (lastId >= 0) {
            ...
            // No messages, but we should still update UI with default/empty state
            this.logger.log(`New chat detected. Updating UI with default state.`);
            this.timeGradient.updateTheme();
            this.uiController.updateAllPanes(); // This is the crucial addition
        }
    ...
    debouncedProcessor(msgId, isReprocessing = false) {
        ...
        this.processorTimeout = setTimeout(async () => {
            ...
            if (isReprocessing && this.previousStateSnapshot) {
                ...
                if (this.timeGradient.isActive) this.timeGradient.updateTheme();
            }
